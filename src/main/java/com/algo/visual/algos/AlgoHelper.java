...
public class AlgoHelper {
    ...
    public void shell(final DrawPanel[] input) {
        ...
        shellTillIncPositive(input, increment);
        finallyColor(input);
    }
    ...
    private void slowDownAndPauseIfNeeded() {
        slowDown();
        pauseThreadIfNeeded();
    }
    ...
    private void shellTillIncPositive(final DrawPanel[] input, int increment) {
        while (increment > 0) {
            visualizer.getNumOfOperations().setText(String.valueOf(++ops));
    ...
    private int maintainInc(int increment) {
        if (increment == 2) {
            visualizer.getNumOfIfs().setText(String.valueOf(++ifs));
            increment = 1;
        } else {
    ...
    private void loopAndIncChange(final DrawPanel[] input, int increment, int i) {
        int j = i;
    ...
    private int incCompareAndChange(final DrawPanel[] input, int increment, int j) {
    ...
    private void finallyColor(DrawPanel[] _lines) {
        for (DrawPanel _line : _lines) {
            _line.setColor(new Color(0, 102, 153));
            pauseThreadIfNeeded();
        }
    }
